...
def make_text_cnn(sentence_size, num_embed, batch_size, vocab_size,
    ...
    if not with_embedding:
        ...
        embed_layer = mx.sym.Embedding(data=input_x, input_dim=vocab_size, output_dim=num_embed, weight=embed_weight, name='vocab_embed')
...
def train_cnn(model, X_train_batch, y_train_batch, X_dev_batch, y_dev_batch, batch_size,
        optimizer='rmsprop', max_grad_norm=5.0, learning_rate=0.0005, epoch=10):
    m = model
    #print >> logs, "%s" % X_train_batch[0]
    # create optimizer
    ...
    for iteration in range(epoch):
	    ...
	    print "iteration[%d],begin[%d]" % (iteration,begin)
        ...
        if (iteration + 1) % 5 == 0:
            prefix = 'cnn'
            m.symbol.save('./example/cnn_text_classification/checkpoint/%s-symbol.json' % prefix)
            save_dict = {('arg:%s' % k) :v  for k, v in m.cnn_exec.arg_dict.items()}
            save_dict.update({('aux:%s' % k) : v for k, v in m.cnn_exec.aux_dict.items()})
            param_name = './example/cnn_text_classification/checkpoint/%s-%04d.params' % (prefix, iteration)
